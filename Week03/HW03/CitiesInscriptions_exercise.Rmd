...
author: "Pete Thramkongart & Jakub Raszka"
...
cities_sf4326<- st_set_crs(cities_sf, 4236)


# Transform the projection to a 2D projection using EPSG 3035
cities_sf3035 <- st_transform(cities_sf4326, crs = st_crs(3035))
...
*Degrees and meters are the measurements*
...
plot(st_geometry(cities_5km)[1:10], col = "yellow") #We use st_geometry() to only get the shape of the objects
...
i_sm <- inscriptions %>%
  ...
  mutate(longitude = as.numeric(str_replace(longitude,  "c\\(", "")),
...
i <- inscriptions %>%
  ...
  mutate(
    longitude = as.numeric(str_replace(longitude,  "c\\(", "")),
    latitude = as.numeric(str_replace(latitude, "\\)", ""))) %>%
...
# Create a sf object
insc_sf4326 <- st_as_sf(i, coords = c("longitude","latitude"), crs = st_crs(4326))
...
insc_sf3035 <- st_transform(insc_sf4326,crs = st_crs(3035))
...
cities_it <- st_intersection(insc_ch, st_buffer(cities_5km, dist = 5000))
...
#get the ratio
insc_urban %>% nrow()/insc_rural %>% nrow()
...
*There are  9.39621 more incsriptions in urban areas than rural areas.*
