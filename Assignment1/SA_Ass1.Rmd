...
Let's imagine a situation which we can only hope to never occur, yet we know has happened, it is happening and it will happen again. In a last year, in the Czech Republic, doctors have examined many patients throughout the republic whose symptoms resemble a severe intoxication. Yet, none of the patients has been in tought with any toxis substances. As the number of patients keeps rising, doctors in colaboration with advanced data specialists, aka cognitive scienticts, are determined to discover the source of poisoning.
Home and work address of all patients are obtained and mapped along with any locations where patients regularly spent substantial amount of time.
...
The hydrography layer is important to map all water areas which could potentially connect a factory and patients through water. The layer of man-made infrastructure is useful when aligning other layers with addresses of patients given by patients.
...
BM_map <- BM_map %>% 
  addCircleMarkers(lng = df$Longitude, 
             ...
             # given the density and immediate proximity of data locations, I think that clusteroptions is a nice improvement in viewing every single datapoint. However, if we just want to get a feeling for the distribution without any info about the data points, the cluster options, should be tagged out
